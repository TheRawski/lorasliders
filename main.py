...
class StringAdjusterApp:
    ...
    def initialize_sliders(self):
        ...
        # Create sliders for each match
        for index, match in enumerate(matches):
            label = tk.Label(self.master, text=f"Slider {index + 1}:")
            label.grid(row=index + 2, column=0, pady=5, padx=10, sticky="w")

            current_value = float(match.split(':')[-1][:-1])
            self.current_values.append(tk.DoubleVar(value=current_value))

            slider = ttk.Scale(self.master, from_=0.0, to=2.0, orient="horizontal", variable=self.current_values[index],
    ...
    def update_text(self, index):
        ...
        for i, match in enumerate(matches):
            ...
            adjusted_text = adjusted_text.replace(match, f'<lora:{match.split(":")[1]}:{current_value:.2f}>', 1)

        self.text_box.delete(1.0, tk.END)
        self.text_box.insert(tk.END, adjusted_text)
        

    def update_text_from_sliders(self):
        # Update the text box with the adjusted values from sliders
        adjusted_text = self.text_box.get(1.0, tk.END)
        matches = re.findall(r'(<lora:\w+:\d+\.\d+>)', adjusted_text)

        for index, match in enumerate(matches):
            current_value = self.current_values[index].get()
            adjusted_text = adjusted_text.replace(match, f'{match[:-3]}{current_value:.2f}>', 1)
...
#### code version below is simpler and does the same thing
